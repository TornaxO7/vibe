...
fn bottom(@builtin(position) pos: vec4f) -> @location(0) vec4f {
    ...
    return mix(color_left, color_right, pos.x / iResolution.x) * get_presence(new_pos, iResolution.y);
}

@fragment
fn top(@builtin(position) pos: vec4f) -> @location(0) vec4f {
    return mix(color_left, color_right, pos.x / iResolution.x) * get_presence(pos.xy, iResolution.y);
}
...
fn right(@builtin(position) pos: vec4f) -> @location(0) vec4f {
    ...
    return mix(color_left, color_right, pos.x / iResolution.x) * get_presence(new_pos, iResolution.x);
}

@fragment
fn left(@builtin(position) pos: vec4f) -> @location(0) vec4f {
    return mix(color_left, color_right, pos.x / iResolution.x) * get_presence(pos.yx, iResolution.x);
}