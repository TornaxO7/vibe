...
// Output:
//   - `x`: The presence
//   - `y`: The relative height within the graph
...
fn bottom(@builtin(position) pos: vec4f) -> @location(0) vec4f {
    ...
    let p = get_presence(new_pos, iResolution.y);
    return mix(color_bottom, color_top, pos.y / (iResolution.y * max_height)) * p;
}

@fragment
fn top(@builtin(position) pos: vec4f) -> @location(0) vec4f {
    let p = get_presence(pos.xy, iResolution.y);
    return mix(color_bottom, color_top, pos.y / (iResolution.y * max_height)) * p;
}
...
fn right(@builtin(position) pos: vec4f) -> @location(0) vec4f {
    ...
    let p = get_presence(new_pos, iResolution.x);
    return mix(color_bottom, color_top, pos.y / (iResolution.y * max_height)) * p;
}

@fragment
fn left(@builtin(position) pos: vec4f) -> @location(0) vec4f {
    let p = get_presence(pos.yx, iResolution.x);
    return mix(color_bottom, color_top, pos.y / (iResolution.y * max_height)) * p;
}
